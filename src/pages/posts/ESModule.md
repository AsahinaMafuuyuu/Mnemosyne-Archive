...
- Node中CJS模块加载采用在 **`服务器端运行时同步加载`** 方式，在 **`浏览器端提前编译打包处理`** 方式
    
- 通过 **`require`** 加载模块，通过 **`exports`** 或 **`module.exports`** 输出模块
	
- CommonJS 规范的核心变量: exports、module.exports、require

require的底层实际上就是 **`读入并执行一个JavaScript文件，然后返回该模块的exports对象。`** ，而且这个返回的对象结果会被缓存下来，再次加载时，**`会从缓存结果中直接读取模块输出结果`**，

cjs同步，esm异步，因为CJS 最初是为服务器端（Node.js）设计的，模块文件都在本地硬盘，读取速度极快，所以“同步阻塞”不会带来明显的性能瓶颈。但 ESM 是为 Web 设计的：

1. **网络延迟：** 如果 `import` 是同步的，浏览器在下载远程服务器上的脚本时必须停止响应，这会导致页面假死。
    
2. **Top-level Await：** ESM 支持在模块顶层直接使用 `await`。这意味着一个模块的执行可以被“暂停”，等待一个 Promise 完成。如果 ESM 是同步加载的，这种特性将无法实现。

因此Cjs通常不允许用于web端：

![](../../assets/images/posts/ESModule-6.png)

require在web端是禁止的，而import却是允许的

> 当前模块会等依赖模块准备好之后才开始执行,因此的话对于import而言，加载资源确实是异步的，但是会暂时存储起来，等资源加载好了才会依次执行后面的模块,比如最简单的代码：`import { a } from './test1.js'; console.log(a)`
> import的资源test1.js是异步加载的，等加载好了才会先执行一遍a，然后将结果缓存下来，然后再执行`const { a } = <cached result>`，然后再执行console.log(a)

> 或者说上面讲那么多，如果不想渗透原理或者也不想深入，只需要记住一点：**node环境下用commonjs，浏览器环境下用ESModule**