...
description: 本文针对博客中的音频播放器进行深入探讨，包括为什么会出现原来的问题，该怎么解决，以及对问题的深思，同时通过这个博客也能够加深对于206状态码相应的各种机制问题，如何来解决cf中发生的情况等
cover:
...
原理其实也就是当cf返回的结果保存为blob二进制对象，然后用该资源创造一个URL，这样的话播放起来会特别流畅，因为就是本地资源，并且增加回收机制，这样的话就不会太占用内存（当然，也可以存储到本地数据库，用IndexDB,下一次播放或者离线播放会更快，这个可以留给以后去优化）

![](../../assets/images/posts/PartialInCf-6.png)

完美的206！

> 虽然这种解决方法对于首次加载音频而言，需要加载整个音频，不过一个音频也才6，7MB左右，显然可以接受，延迟大概在3，4秒左右，显然是可以接受的，并且由于cf中对于CDN有优化，所以还算可以接收（不过也很显然有一个问题：如果每一次进行加载的时候都要去通过带宽重新请求的话，这个流量负荷就不小），因此后续打算进行以下优化：
> 1. 已经缓存过的首先存储在内存当中，通过映射表来进行直接访问
> 2. 对于这种静态资源，后续存储在内存当中，内存访问比网络快很多，并且不消耗服务器带宽，并且占用内存可以选择GZIP等等压缩，这样的话占用存储也不大，并且也不占用宝贵的内存

![](../../assets/images/posts/PartialInCf-7.png)

> 对于用户访问量不大的情况下，这个加载大小勉强可以接受的（反正我这个博客也没什么人看），不过如果访客量每秒100个的话，每秒300MB的带宽，那确实有点哈人，后续进行一系列的优化吧。