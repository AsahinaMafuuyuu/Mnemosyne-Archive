...
astro中根据变化前后的transition:name来进行相应的匹配，并且制作动画过渡：

![](../../assets/images/posts/AstroTransitionAnimation.png)

上面旧页面和新页面的aside组件具有相同的transition:name，因此进行动画过渡的时候会考虑将这一对进行过渡动画（**但是同一个页面不能有两个组件具有相同的transition:name!!!**）